...
#Testingurl = ('http://0.0.0.0:%s' %(sys.argv[1])) # testing local server
UploadUrl = "http://192.168.0.4:9128/api/ams/devices/let-me-in"
...
# Initialize the LCD using the pins above.
	lcd.set_backlight(0)
	str_pad = " " * 16
	CompanyName = str_pad + 'Rydot Infotech Ltd.'
	for i in range (0, (len(CompanyName)+1)):
		lcd.clear()
		lcd_text = CompanyName[i:(i+16)]
...
def Setup():
	...
	LCDInit()   
	SystemRegistration() # System Registration Started
...
def SystemRegistration():
	...
	global ucContent
	lcd.clear()
	lcd.message('System Registration\nStart')
	...
	if j['success'] == False:
		...
		lcd.clear()
		lcd.message('System Registration\nSucessful')
		# GetConfiguration() # If Registration Sucess then calling GetComfiguration.
...
def GetConfiguration():
	...
	data = "{\"apiKey\":"+('\"%s\",' %ApiKey)+"\"apiSecret\":"+('\"%s\"}' %ApiSecret)
	print data
	...
	# print Config
	VideoLength = tuple(jsonnn_tree.execute('$..videoLength'))
	ucMediaType = tuple(jsonnn_tree.execute('$..imageType'))
	print 'VideoLength : %d' %VideoLength
	print 'MediaType : %s' %ucMediaType

	# Typecast needed otherwise got "TypeError: unsupported operand type(s) for -: 'tuple' and 'int'"
	for VideoLength in VideoLength:
		print(VideoLength)
		gOut.writerow([('%s' %(str(dt.now()))),'VideoLength','%d' %VideoLength])
	for ucMediaType in ucMediaType:
		print(ucMediaType)
