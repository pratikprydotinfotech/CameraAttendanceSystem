...
REG_VIDEO_DIR_PATH = abs_path+"RegVideo"
...
def LogFileGeneration():
	...
	os.system('rm -rf RegVideo') # Registration directory remove
	os.mkdir( REG_VIDEO_DIR_PATH , 0777) # Registration directory generate
...
# @ Parameter   : void
# @ Return      : void
# @ Brief       : Read Registration Data from registration.js
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def ReadFileRegistration():
	RegFile = open("registration.js", "rb")
	RegContent = RegFile.read().splitlines()
	if RegContent == []:
		SystemRegistration()
	...
	serialnumber = RegContent[0]
	apikey = RegContent[1]
	apisecret = RegContent[2]
	print serialnumber
	print apikey
	print apisecret
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# @ Parameter   : void
# @ Return      : void
# @ Brief       : Read Configuration Data from Configuration.js
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def ReadFileConfiguration():
	ConfFile = open("configuration.js", "rb")
	ConfContent = ConfFile.read().splitlines()
	if ConfContent == []:
		GetConfiguration()
	...
	videoLength = ConfContent[0]
	ucMediaType = ConfContent[1]
	Enabled = ConfContent[2]
	giCurrentEmployee = ConfContent[3]
	giNumPhotos = ConfContent[4]
	giRegEmployeeName = ConfContent[5]
...
# @ Return      : void
# @ Brief       : Write Registration Data from registration.js
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def WriteFileRegistration(serialnumber,apikey,apisecret):
	RegFile = open(abs_path+"registration.js","w+")
	RegFile.write('%s\n' % serialnumber)
	RegFile.write('%s\n' % apikey)
	RegFile.write('%s\n' % apisecret)
	RegFile.close()
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# @Function 	: WriteFileConfiguration()
# @ Parameter   : string,string,string
# @ Return      : void
# @ Brief       : Write Configuration Data from Configuration.js
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def WriteFileConfiguration(VideoLength,ucMediaType,Enabled,giCurrentEmployee,giNumPhotos,giRegEmployeeName):
	ConfFile = open(abs_path+"configuration.js","w+")
	ConfFile.write('%s\n' % VideoLength)
	ConfFile.write('%s\n' % ucMediaType)
	ConfFile.write('%s\n' % Enabled)
	ConfFile.write('%s\n' % giCurrentEmployee)
...
def Setup():
	...
	if ucMediaType == "moving": # For Image
		giFlag = 0
		READ_SWITCH = 21
	...
	wiringpi.pullUpDnControl(led_pin,0)
...
def SystemRegistration():
	...
	serialnumber = j['body']['serialNumber']
	apikey = j['body']['apiKey']
	apisecret = j['body']['apiSecret']

	#Write Registration Data from registration.js
	WriteFileRegistration(serialnumber,apikey,apisecret)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# @Function 	: UserImageRegistration()
# @ Parameter   : void
# @ Return      : void
# @ Brief       : Empolyee Registration Function 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def UserImageRegistration():
	global giRegFlag
	global Images
	global giNumPhotos
	global giCurrentEmployee
	...
	RegImg = 0
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# @Function 	: UserVideoRegistration()
# @ Parameter   : void
# @ Return      : void
# @ Brief       : Empolyee Registration Function 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def UserVideoRegistration():
	global giRegFlag
	global Images
	global giNumPhotos
	global giCurrentEmployee
	...
	lcd.clear()
	lcd.message('wait for\nProcessing...')
	print ('USER_REGISTRATION_START>>>>>>>>>>>>>>>>>>>>>>>>>>')
	gOut.writerow([('%s' %(str(dt.now()))),'USER_REGISTRATION_START'])
	storage = StringIO()
	c = pycurl.Curl()
	c.setopt(c.URL, UserRegisterUrl)
	c.setopt(c.WRITEFUNCTION, storage.write)
	c.setopt(c.POST, 1)
	c.setopt(c.HTTPHEADER,['x-api-key: key-9acc61be5b2fe7f347908a9ced1f638f','x-api-secret: secret-835e6ed8dbcffcb7fa00a7b22be39e5e'])
	send = [("files", (c.FORM_FILE,abs_path+'Regclip.mp4'))]
	c.setopt(c.HTTPPOST,send) 
	c.perform()
	content = storage.getvalue()
	j = json.loads(content)
	gOut.writerow(['Registration: %s' % content])
	if j['success'] == True:
		print ('Registration Success for %s Employee' %giCurrentEmployee)
		gOut.writerow([('%s' %(str(dt.now()))),'Registration Success for %s Employee' % giCurrentEmployee])
		lcd.clear()
		ScrollingLCD('Congratulation, %s' %giRegEmployeeName)
	elif j['success'] == False:
	...
	c.close()
	os.system('rm -rf RegVideo') # Registration directory remove
	os.mkdir( REG_VIDEO_DIR_PATH , 0777)
	os.system ('rm -rf Regclip.mp4')	
	giRegImgCount = 0
	giRegFlag = 0
	giFlag = 0
	giOpenFlag = 0
...
def GetConfiguration():
	...
	# ucMediaType = "moving" # For testing change Media Type here
	# VideoLength = 3

	# Write Configuration Data from configuration.js
	WriteFileConfiguration(VideoLength,ucMediaType,Enabled,giCurrentEmployee,giNumPhotos,giRegEmployeeName)
...
def SendData(RecPath): 
	...
	c.setopt(pycurl.CONNECTTIMEOUT, 60) # Timeout 1 second
...
# Try and Exception for Server Timeout 
	...
	try:
		...
		if errstr == "Empty reply from server":
			lcd.clear()
			lcd.message("Sorry,Please\ntry again.")
			time.sleep(1)
			lcd.clear()
			lcd.message('Press Switch for\ntake a picture')
		elif errstr == "Failed to connect to 104.237.4.152 port 9128: Connection refused":
			giTimeCount+=1
	...
	# Try and Exception for Status of Server 
		...
		if ucMediaType == "moving" and j['success'] == True:
			gOut.writerow([('%s' %(str(dt.now()))),'%s' %(j['message'])])
			if j['message'] == "Sorry authentication failed.":
				...
				lcd.clear()
				ByeString = j['message']
				ByeString = ByeString.replace('Welcome','GoodBye')
				ScrollingLCD(ByeString)
				time.sleep(1)
				lcd.clear()
				lcd.message('***Thank you***')
				time.sleep(2)
		elif ucMediaType == "moving" and j['success'] == False:
			lcd.clear()
...
def CaptureVideo():		
			...
			global VideoLength
			global giOpenFlag
			gCamera.annotate_text_size = TEXT_SIZE
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# @Function 	: RegVideoRecord
# @ Parameter   : void
# @ Return      : void
# @ Brief       : This Function is Register Capturing Video
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def RegVideoRecord():		
			global RegImg
			global gCamera
			#gCamera.annotate_text = ('Rydot infotech Attendance System_'+('%s' %(str(dt.now()))))
			lcd.clear()
			lcd.message('Look infront\nof Camera')
			gCamera.brightness = BRIGHTNESS
			gCamera.resolution = (RESOLUTION_H, RESOLUTION_W) 
			gCamera.framerate = VIDEO_FRAMERATE
			gOut.writerow([('%s' %(str(dt.now()))),'Video_Recording_for_Registration_Start'])
			gCamera.start_recording(abs_path+'RegVideo/video%s.h264' %RegImg)
			RegVideoRawPath = abs_path+'RegVideo/video%s.h264' %RegImg
			time.sleep(3)
			gCamera.stop_recording()
			gOut.writerow([('%s' %(str(dt.now()))),'Video_Recording_for_Registration_Stop'])
			os.system('MP4Box -fps 30 -add '+RegVideoRawPath+' Regclip.mp4')
			UserVideoRegistration()
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
def VideoRecord_attendance():		
			...
			gCamera.framerate = VIDEO_FRAMERATE
			gOut.writerow([('%s' %(str(dt.now()))),'Video_Recording_for_Registration_Start'])
			gCamera.start_recording(abs_path+'Raw_Video/video%s.h264' %RegImg)
			RegVideoRawPath = abs_path+'Raw_Video/video%s.h264' %RegImg
			time.sleep(0.5)
			gCamera.stop_recording()
			gOut.writerow([('%s' %(str(dt.now()))),'Video_Recording_for_Registration_Stop'])
			MP4_Rec_Path = abs_path+'MP4_Video/Regclip.mp4'
			print MP4_Rec_Path
			os.system('MP4Box -fps 30 -add '+RegVideoRawPath+' '+MP4_Rec_Path+'')
			time.sleep(3)
...
def DoorEventThread():
	while True:
		...
		global ucMediaType
		print("In thread1")
		if wiringpi.digitalRead(READ_SWITCH) == 1 and count<=10 and giRegFlag != 1 and ucMediaType == "moving":
			giFlag = 0
			count+=1
			print 'count:%s' %count
			time.sleep(1)
			...
			if count >=10:
				...
				GetConfiguration()
				count = 0
		elif wiringpi.digitalRead(READ_SWITCH) == 1 and count<=10 and giRegFlag == 1 and ucMediaType == "moving":
			giFlag = 0
			count+=1
			print 'count:%s' %count
			time.sleep(1)
	...
	while True:
		...
		if (len(PendingImages)) > 0 and iServerResponse == True:
			...
			if i == ((len(PendingImages))-1):
				...
				gOut.writerow([('%s' %(str(dt.now()))),'PendingImage_Buffer',len(PendingImages)])
				giPendingCount = 0 
				if ucMediaType == 'moving' and giRegFlag == 0:
					lcd.clear()
					lcd.message('Press Switch for\ntake a picture')
				gOut.writerow([('%s' %(str(dt.now()))),'PendingImage_Buffer',len(PendingImages)])
		# else:
