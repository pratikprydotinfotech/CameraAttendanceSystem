...
def LogFileGeneration():
	...
	os.system('rm -rf RegImage') # Image directory remove 
	os.mkdir( REG_IMAGE_DIR_PATH, 0777 )  # New MP$_Video directory generate
...
def UserRegistration():
	...
	if j['success'] == True:
		gOut.writerow([('%s' %(str(dt.now()))),'Registration Sucess for %s Employee',giCurrentEmployee])
...
def ImageLoop():
			...
			# 		RegImg+=1
			# 		giFlag = 0
			# 	elif giRegImgCount >= giNumPhotos:
			# 		UserRegistration()
			# 		os.system('rm -rf RegImage') # Image directory remove 
			# 		os.mkdir( REG_IMAGE_DIR_PATH, 0777 )  # New MP$_Video directory generate
			# 		giFlag = 0
			# 		RegImg = 0
			# 		lcd.clear()
			# 		ScrollingLCD('Congratulation, Registration done')
			# 		lcd.clear()
			# 		lcd.message('Press Switch for\ntake a picture')
			# elif giRegFlag == 1 and giFlag == 0:
			# 	lcd.clear()
